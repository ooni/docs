...
# version: ooni/data/docs/architecture.md:b11357f
title: "Architecture"
description: "How the OONI data pipeline is put together: processing tiers, infrastructure and tradeoffs"
slug: data/pipeline
sidebar:
    order: 1
---
[edit file](https://github.com/ooni/data/edit/main/docs/architecture.md)
...
[Requirements](/data/pipeline-design) (what it must achieve),
[Ontology](/data/pipeline-ontology) (what the entities mean),
[User guide](/data/pipeline-user-guide) (consuming the output),
[Developer guide](/data/pipeline-developer-guide) (working on it),
[Implementation plan](/data/pipeline-implementation-plan) (current state and next steps).
Bare ids like `P2` or `O3` cite [Requirements](/data/pipeline-design).

**Status markers**: **[built]** is in production, **[partial]** exists but is
not fully wired, **[mvp]** is in the current delivery scope, **[later]** is
deferred (see [Ontology](/data/pipeline-ontology) Appendix A). Unmarked means built.
...
Retiring it is tracked in [Implementation plan](/data/pipeline-implementation-plan) §4.
...
consensus, sample size and ambiguity all derive ([Ontology](/data/pipeline-ontology) §9).
...
stateful (see [Developer guide](/data/pipeline-developer-guide)).
...
- `obs_tunnel` **[mvp]**: one row per (measurement, endpoint, phase) for
  ...
  [Ontology](/data/pipeline-ontology) §2, §3.1.
...
- per-connection or provider-aggregated. Outside the tiered chain:
  joins no control set. [Ontology](/data/pipeline-ontology) §5.2.
...
- Cell state: a view over tier 2, not a table. §3.1 below; semantics in
  [Ontology](/data/pipeline-ontology) §9. Extends to tunnel targets at the same grain,
...
- `event_detector_cusums`: per `(cc, asn, domain)` CUSUM accumulator state.
  ...
  ([Ontology](/data/pipeline-ontology) §11); no new table, same accumulator shape.
...
[Ontology](/data/pipeline-ontology) §9; `n_probes` counts distinct credentialed probes
...
([Ontology](/data/pipeline-ontology) §5.1). Unlike the web control, this cannot be
...
[Requirements](/data/pipeline-design)'s assessment vocabulary.
...
[Developer guide](/data/pipeline-developer-guide)).
...
[Developer guide](/data/pipeline-developer-guide) §6.1.
...
see [Implementation plan](/data/pipeline-implementation-plan) §3.3.
