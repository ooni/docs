...
# version: ooni/data/docs/developer-guide.md:69e7e11
title: "Developer guide"
description: "Working on the pipeline: local setup, common changes and the traps"
slug: data/pipeline-developer-guide
sidebar:
    order: 4
---
[edit file](https://github.com/ooni/data/edit/main/docs/developer-guide.md)
...
[Implementation plan](/data/pipeline-implementation-plan).
Bare ids like `O2` or `V2` cite [Requirements](/data/pipeline-design).
...
- **Do not change existing weights casually.** A weight change must be shown
  ...
  [analysis-evaluation.ipynb](https://github.com/ooni/data/blob/main/docs/analysis-evaluation.ipynb) and ship the diff
...
The adjudication tool is [labeler.html](/tools/labeler): a self-contained page,
...
4. **Label.** One of `blocked` / `down` / `ok` / `can't call it` / `unusable`,
   ...
   [Ontology](/data/pipeline-ontology) §12: censorship co-occurs (DNS injection next to
