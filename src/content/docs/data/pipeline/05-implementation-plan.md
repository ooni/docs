...
# version: ooni/data/docs/implementation-plan.md:7ea0f62
title: "Implementation plan"
description: "Where the pipeline stands today and what the next steps are"
slug: data/pipeline-implementation-plan
sidebar:
    order: 5
---
[edit file](https://github.com/ooni/data/edit/main/docs/implementation-plan.md)
...
[Requirements](/data/pipeline-design)), and §5 states this plan's tradeoffs as
requirement conflicts.

Companion documents: [Requirements](/data/pipeline-design),
[Architecture](/data/pipeline),
[Ontology](/data/pipeline-ontology), [User guide](/data/pipeline-user-guide),
[Developer guide](/data/pipeline-developer-guide). Live-cluster migrations are in
[Migration notes](/data/pipeline-migration-notes).
...
Both labelling tools are built: [labeler.html](/tools/labeler) (workflow in
[Developer guide](/data/pipeline-developer-guide) §3.5) and
[event-labeler.html](/event-labeler.html)
([Label corpus](/data/pipeline-label-corpus)). The first draws exist
under [labels/](https://github.com/ooni/data/tree/main/docs/labels): measurement labels from two adjudicators plus an
imported event-label draft. The corpus is still far from the volume targets in
[Label corpus](/data/pipeline-label-corpus) §2.5, and until §3.9 persists the fired-rule sets, the
...
in [Ontology](/data/pipeline-ontology) Appendix A with the trigger that would revive
...
[Ontology](/data/pipeline-ontology) §10.
...
networks, which is future work ([Ontology](/data/pipeline-ontology) §11).
...
histogram (canonical query in [Architecture](/data/pipeline) §3.1; semantics
in [Ontology](/data/pipeline-ontology) §9). Consensus, sample size and ambiguity all fall
...
([labeler.html](/tools/labeler); workflow in
[Developer guide](/data/pipeline-developer-guide) §3.5). The design decisions that make
...
- **Labels use the mechanism taxonomy, not rule ids**
  ([Ontology](/data/pipeline-ontology) §12): any prefix of a taxonomy path is a valid
...
in [Label corpus](/data/pipeline-label-corpus) §1.2: scope with explicit
...
[Developer guide](/data/pipeline-developer-guide) §4.3.
...
multi-resolver endpoints are analysable ([Ontology](/data/pipeline-ontology) §6.2). What
...
([Ontology](/data/pipeline-ontology) §5). A poisoned entry rewrites the scoring of
...
3. **Central revalidation** is the durable fix, blocked on a storage decision:
   ...
   ([Ontology](/data/pipeline-ontology) Appendix A.5 records the trigger).
...
Land `obs_tunnel` ([Ontology](/data/pipeline-ontology) §2), converging the OpenVPN
...
[Ontology](/data/pipeline-ontology) §4.1), build the cross-network baseline aggregate
...
([Ontology](/data/pipeline-ontology) §11), through the same cell state, detector and
...
|---|---|
| Per-rule likelihood-ratio calibration (E6) | Underway rather than deferred: the fit lives in [analysis-evaluation.ipynb](https://github.com/ooni/data/blob/main/docs/analysis-evaluation.ipynb) and the API's `Calibration` reads its output (migration-notes §5). Full promotion needs 3.9's fired sets, or the LRs stay conditional on "fired **and won**", a cascade-position-dependent quantity. |
...
| External reports tier (M8, C9) | `external_reports` table and `vpnext` transformer, corroboration join at event grading only ([Ontology](/data/pipeline-ontology) §5.2, Appendix A.7). Trigger: a signed provider agreement under the published report format, with PR1's retention, access and aggregation-floor decisions settled first. This is where a companion project's provider-telemetry relationship (if one exists) would arrive pipeline-side: that project owns the collection and the relationship, this plan owns what the data may and may not mean once it does. |
| Tunnel mechanism taxonomy leaves (`tunnel.*`) | Reserved, not scored ([Ontology](/data/pipeline-ontology) §12.3). Trigger: first adjudicated tunnel labels (3.8-style corpus extension) or alignment with an external taxonomy effort. |
...
[Requirements](/data/pipeline-design).
