...
# version: ooni/data/docs/label-corpus-design.md:b11357f
title: "Label corpus"
description: "The ground-truth corpora used to evaluate scoring and detection"
slug: data/pipeline-label-corpus
sidebar:
    order: 6
---
[edit file](https://github.com/ooni/data/edit/main/docs/label-corpus-design.md)

Two label grains, one curation workflow. This is the machinery behind
requirements V1–V3 ([Requirements](/data/pipeline-design)): ground truth
...
per-rule likelihood ratios in [analysis-evaluation.ipynb](https://github.com/ooni/data/blob/main/docs/analysis-evaluation.ipynb)
...
Emitted by [labeler.html](/tools/labeler). One object per adjudication, append-only:
...
`mechanisms` is an array under taxonomy v1 ([Ontology](/data/pipeline-ontology) §12).
...
- `scored_rules` (the full fired set) needs the persistence work in
  [Implementation plan](/data/pipeline-implementation-plan) §3.9. Until then LRs are
...
incidents by [incidents_to_events.py](https://github.com/ooni/data/blob/main/scripts/incidents_to_events.py).
...
[Implementation plan](/data/pipeline-implementation-plan) §3.15 landing first: there is
...
weight ships** ([Implementation plan](/data/pipeline-implementation-plan) §3.15's V2
...
[labeler.html](/tools/labeler). One row at a time, drawn from a stratum,
...
**Trigger:** [Implementation plan](/data/pipeline-implementation-plan) §3.15 landing,
...
| | Workflow | Status |
|---|---|---|
| W1 | Fired-rule persistence | **not done**: only the winning rule per layer is stored, so LRs are conditional on "fired and won" ([Implementation plan](/data/pipeline-implementation-plan) §3.9) |
...
| W7 | The LR fit | **done**: [analysis-evaluation.ipynb](https://github.com/ooni/data/blob/main/docs/analysis-evaluation.ipynb) §5, conditioned per layer, [Jeffreys-smoothed](https://en.wikipedia.org/wiki/Jeffreys_prior), bootstrap CIs. Not done: beta-binomial shrinkage, probe clustering |
